...
SCHEMA_VERSION = 2  # v2 (2026-06-21): post-hoc BH-FDR via apply_corpus_bh_fdr.py
BASELINE_KINDS = ("best_of_n", "n_rank", "n_steps", "n_data")
METRICS = ("accuracy", "ece")  # brier needs full prob distribution; deferred
# NOTE: per-cell writes here carry only `holm_adjusted_p_singleton` (a one-cell
# Holm — i.e. p itself, since correction across a single test is a no-op) and a
# CI-based `adjudication`. Corpus-wide BH q-values and the paper-aligned
# post-FDR `adjudication` field are populated by
# `scripts/apply_corpus_bh_fdr.py` once the full cell batch is on disk.
# Until that batch script has been run, the `adjudication` field in a freshly
# written cell reflects CI only and must NOT be cited as the post-FDR label.
