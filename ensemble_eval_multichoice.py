...
adapters via:
  ...
  - greedy_soup (greedy subset selected on val_selection accuracy)
  - uniform_soup (prediction-space uniform logit average; weight-space
                  merge is done separately by weight_space_merge.py)
  - best_single (highest val_selection accuracy)
...
def main() -> int:
    ...
    # Split discipline (paper/prereg.md §3):
    #   val_selection — pick ensemble members & soup weights
    #   val_combine   — tune combination-method hyperparameters only
    #   test          — final eval only
    # Audit fix (Critical 1): previous version selected best_single, greedy_soup
    # members, and per_adapter_val_acc on val_combine, which leaked the
    # combine split into selection. We now use val_selection for all selection
    # decisions and retain per_adapter_combine_acc only as a transparency
    # field (NOT used for selection).
    val_sel = splits["val_selection"]
    val_com = splits["val_combine"]
    test = splits["test"]
    per_adapter_sel_acc = np.array([
        (L[m, val_sel].argmax(1) == labels[val_sel]).mean() for m in range(M)
    ])
    per_adapter_combine_acc = np.array([
        (L[m, val_com].argmax(1) == labels[val_com]).mean() for m in range(M)
    ])
    # Back-compat alias kept for downstream scripts that still read this key.
    # NEW SEMANTICS: per_adapter_val_acc == per_adapter_sel_acc (val_selection).
    per_adapter_val_acc = per_adapter_sel_acc
    ...
    if "best_single" in methods:
        # Selection on val_SELECTION (audit fix Critical 1).
        m_best = int(per_adapter_sel_acc.argmax())
        p = P[m_best, test]
        add_method("best_single", p.argmax(1), p, p.max(1))
        results["best_single"]["selected_adapter_idx"] = m_best
        results["best_single"]["selection_split"] = "val_selection"
    ...
    if "greedy_soup" in methods:
        # Greedy subset selection on val_SELECTION accuracy (audit fix Critical 1).
        # Previous version used val_combine, which violated prereg split discipline.
        order = np.argsort(-per_adapter_sel_acc)
        chosen = [int(order[0])]
        best_acc = (P[chosen[0], val_sel].argmax(1) == labels[val_sel]).mean()
        for cand in order[1:]:
            test_set = chosen + [int(cand)]
            mean_p = P[test_set].mean(axis=0)
            acc = (mean_p[val_sel].argmax(1) == labels[val_sel]).mean()
            if acc > best_acc:
                chosen = test_set
                best_acc = acc
        ...
        results["greedy_soup"]["selection_split"] = "val_selection"

    if "uniform_soup" in methods:
        # PROXY: prediction-space uniform logit average. Audit fix Critical 2:
        # this is NOT the weight-space LoRA soup; real weight-space soup
        # results live in merged_adapters/ via weight_space_merge.py.
        l_uni = L.mean(axis=0)[test]
        p = softmax(l_uni, axis=-1)
        add_method("uniform_soup", p.argmax(1), p, p.max(1))
        results["uniform_soup"]["method_kind"] = "prediction"
        results["uniform_soup"]["is_proxy"] = True
        results["uniform_soup"]["proxy_for"] = "weight_space_uniform_soup"
        results["uniform_soup"]["note"] = (
            "prediction-space uniform logit average; weight-space LoRA soup is "
            "a separate artifact produced by weight_space_merge.py"
        )

    # Tag remaining methods' method_kind for review parity with ensemble_eval.py.
    for _k in ("soft_vote", "logit_avg", "majority_vote", "greedy_soup"):
        if _k in results and "method_kind" not in results[_k]:
            results[_k]["method_kind"] = "prediction"
    for _k in ("best_single",):
        if _k in results and "method_kind" not in results[_k]:
            results[_k]["method_kind"] = "selection"
    ...
    out = {
        ...
        "per_adapter_sel_acc": per_adapter_sel_acc.tolist(),
        "per_adapter_combine_acc": per_adapter_combine_acc.tolist(),
        "selection_split": "val_selection",
        "combine_split": "val_combine",
        "audit_fix_critical_1": "selection now uses val_selection (was val_combine); see scripts/apply_corpus_bh_fdr.py + git log for history",
