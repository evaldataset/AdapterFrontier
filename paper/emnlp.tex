...
\author{
  ...
  \texttt{anonymous@example.\allowbreak org}
}
...
\section{Benchmark composition}\label{sec:setup}
...
\paragraph{The strict comparison first.} Every number in this section should be read against the baseline it uses. Against the pre-registered \texttt{n\_rank} single adapter --- which measured from the released manifests received a median $1.45\times$ the pool's training wall-clock (max $2.96\times$, $47/48$ pairs; \texttt{analysis/\allowbreak baseline\_\allowbreak budget\_\allowbreak audit.\allowbreak json}) plus a held-out hyperparameter search the ensemble never gets, and therefore acts as a \emph{strict upper bound} rather than a matched competitor --- the clean encoder slice contains \textbf{no} cell with a significant accuracy gain ($0$ of $92$, before and after FDR) and $33.7\%$ significant reversals. On the \emph{same} $92$ cells the calibration arm runs the other way: $52$ SUPPORTED, $17$ REVERSED. The patterns P$_a$--P$_c$ above, and much of the literature they echo, are stated against the pool's own best member (\texttt{best\_of\_n}), a strictly weaker comparison. \emph{Precision.} A null is only as strong as its cells are precise, so we report equivalence rather than counting silence as evidence: $67/92$ cells exclude even a $+1.0$pp gain. The strength sits where it should --- on the $68$ well-powered cells ($n_{\text{test}}\!\geq\!2000$) the median CI half-width is $0.91$pp and $62$ exclude $+1.0$pp, while the $24$ small-$n$ cells (median $3.85$pp) are uninformative in \emph{either} direction and are counted for transparency, not as evidence. \emph{Procedure dependence.} The accuracy null is invariant to the correction rule (SUPPORTED $=0$ under raw CI, within-table Holm and corpus-wide BH alike); the REVERSED rate is not ($41.3/17.4/33.7\%$), and calibration survives all three ($52/46/52$). A fourth procedure, corpus-wide Holm, returns $0/0$ in both arms, but that row is resolution-limited rather than evidential --- its threshold $\alpha/m{=}4.86{\times}10^{-5}$ sits below the smallest $p$ our Monte-Carlo permutation can produce, so no cell could pass at any effect size (App.~\ref{app:correction}). Full grid in App.~\ref{app:correction}. We quantify that gap, and three further protocol choices, in \S\ref{sec:protocol}. What follows characterises \emph{where} in the corpus the deltas sit and how predictable they are --- not a claim that ensembling pays.

\paragraph{The corpus as a whole, and why we headline a slice of it.} Across all $1{,}028$ cells the picture is mixed, not negative: $33.0\%$ SUPPORTED and $17.8\%$ REVERSED pre-correction, $29.5\%$ and $16.9\%$ after corpus-wide BH-FDR. We do not headline it, because most of it rests on cells compared against the pool's own best member --- the weak baseline of \S\ref{sec:protocol} --- and on families we disqualify below. The $92$-cell slice we headline is the intersection of three pre-registered conditions (compute-matched \texttt{n\_rank}, encoder classification, accuracy): the strictest cut the corpus supports, and the one where it disagrees with the literature.

\paragraph{Where the deltas sit (Table~\ref{tab:v11cells}).} Against the pool's own best member (\texttt{best\_of\_n}) the converged encoder pools do show the pattern the literature reports: on BERT-base Pool-A/B/C soft averaging beats it by $+0.9$ to $+1.0$pp while \texttt{greedy\_soup} merely matches it, and scale erodes even that (BERT-base$\to$large drops the gain to $+0.65$pp and degrades ECE from $0.048$ to $0.118$). Against the compute-matched \texttt{n\_rank} single, the \emph{same cells} carry no significant gain --- which is shortcut S2 (\S\ref{sec:protocol}) measured on our own corpus rather than argued in the abstract. Two families are excluded from every headline statistic: the HellaSwag pools, scored by a pre-fix evaluator on the wrong split (App.~\ref{app:hellaswag}, L1b), and the decoder GSM8K pools, whose \texttt{best\_single} was selected on the split it is evaluated on and which therefore support no positive claim in either direction (App.~\ref{app:gsm8k-scaling}, L12).
...
\begin{table}[h]
\centering
\caption{Cross-model-class LOPO-CV $R^2$, identical slices and folds (\texttt{analysis/\allowbreak frontier\_\allowbreak model\_\allowbreak classes.\allowbreak py}). Ridge is the cross-class maximum on the headline slice and on that slice only; non-linear models win the other three, so reporting ridge throughout costs us $R^2$ rather than buying it.}
...
The corpus lets us ask a question the ensembling literature has not asked directly: \emph{how much of the reported benefit is produced by the evaluation protocol rather than by ensembling?} We isolate three shortcuts (Table~\ref{tab:protocol}), each common in published practice, and measure what each is worth; all three are recomputed from released artifacts alone. A shortcut here is not a mistake that always flatters ensembling --- it is a degree of freedom that flatters whichever arm is given it. We report each one's size and say which arm it favours.

\paragraph{S1: selecting the single adapter on test.} A pool's ``best single'' is often reported at its test-set maximum. Reconstructing that oracle from \texttt{individual\_\allowbreak accuracy\_\allowbreak test} and comparing against the held-out \texttt{val\_selection} choice, test-selection is worth $+1.12$pp on average ($+1.30$pp on encoders; median $+0.57$, max $+9.58$) and inflates $82\%$ of pools. This is a free gain awarded to whichever arm is allowed to peek. In our own corpus the arm that peeks is the \emph{single} adapter, so S1 makes the ensemble look \emph{worse}; in the literature the same freedom is more often given to the ensemble's member selection, where it works the other way. Either way the quantity is the same $+1.12$pp, and it is unearned.
...
One caveat the corpus itself exposes: the two baselines do not cover identical pool sets ($80$ pools have \texttt{best\_of\_n}, $48$ have \texttt{n\_rank}, $48$ have both). On the $48$ carrying both, the drop is $24.5\to9.9\%$, so about a sixth of the headline effect is pool composition rather than baseline strength. We report the matched figure alongside the corpus-wide one rather than only the larger.
...
\paragraph{Escape routes are narrow.} Two standard escapes are limited here. \emph{Distillation} \citep{hinton2015distilling} recovers ensemble accuracy on only $1$ of $4$ cells ($87\%$ on Qwen-0.5B MNLI; $4.5\%$ on Qwen-1.5B; failure on BERT-base; $-181\%$ on BERT-large, which still improves ECE to $0.019$ from $0.035$; App.~\ref{app:distill}). \emph{Serving} the pool is cheap in wall-clock (sequential-inference latency in App.~\ref{app:latency}; costs in App.~\ref{app:cost}) (vLLM~0.10 at $N{=}8$: $1.075\times$ on Qwen-0.5B, $1.067\times$ on Llama-3.1-8B-Instruct; App.~\ref{app:vllm}), so the barrier is not latency but the absence of an accuracy gain to serve. Accuracy plateaus at $N{\approx}8$--$16$, and on ANLI soft-vote helps on only $5/19$ MNLI pools (App.~\ref{app:scaling},~\ref{app:ood}).

\paragraph{When a population might still pay.} Our null is bounded, and the bounds are where a practitioner should look. It is an \emph{encoder classification} result at pool sizes $N\!\leq\!24$ and adapter budgets of rank $16$--$32$ over $2$--$4$ epochs, and it is conditional on spending the extra budget on \emph{rank}: the pre-registered \texttt{n\_steps} and \texttt{n\_data} allocations were filled in only $3$ and $0$ cells respectively, so a single adapter given the same budget as more steps or more data is untested here and could be weaker. Three situations lie outside what we measured: pools far larger than ours, where the plateau we see at $N\!\approx\!8$--$16$ may not be the ceiling; members differing by more than seed, hyperparameters or PEFT method, since our diversity is manufactured rather than found; and deployments where no single adapter can be retrained, which removes the comparison we run. We expect the null to hold wherever a stronger single \emph{is} trainable, and would rather it be tested there than assumed.

\paragraph{Takeaway.} Under a fixed budget, a single adapter trained with the pool's compute is the right default, and the recommendation we can defend is cheaper than ensembling rather than a version of it: \emph{train one adapter with the budget and fit one temperature on held-out data}. That buys the reliability the ensemble appeared to buy, at $1\times$ inference instead of $N\times$, and it is the arm the literature --- and, until we ran the control, this paper --- left out. Our D1--D5 rules were derived and evaluated on this corpus, so they are an oracle upper bound (L11): a prospective, validation-only version degenerated to always choosing the single adapter.

\FloatBarrier
\section*{Limitations}

\paragraph{What this paper does \emph{not} claim.} App.~\ref{app:delta} sets what is new here against prior PEFT-ensembling work, cell by cell. For reviewer transparency, we enumerate the inferences that our 1{,}028 cells do \emph{not} support: \textbf{(a)} we do \emph{not} claim a single best combination method --- when a pool is worth combining at all, \texttt{greedy\_soup} is the least-bad accuracy option on under-converged encoders and \texttt{soft\_vote} the best calibrated on converged ones; the apparent \texttt{majority\_vote} advantage on $\geq 1.5$B decoders rests on GSM8K and does not survive decontamination (L12), so we withdraw it; \textbf{(b)} we do \emph{not} claim ensembling is universally beneficial --- $17.8\%$ of cells are REVERSED pre-FDR ($16.9\%$ post-FDR at $q{=}0.05$), and most v1.0 encoder accuracy cells are reversed against \texttt{n\_rank}; \textbf{(c)} we do \emph{not} claim our diversity-quality frontier transfers to other PEFT families (LoHa, AdaLoRA, prompt tuning) or to instruction-tuned base models; \textbf{(d)} we do \emph{not} claim distillation generalizes --- accuracy recovery: Qwen-0.5B $87\%$, Qwen-1.5B $4.5\%$, BERT-base fails, BERT-large $-181\%$ ($1$-of-$4$ cells positive); \textbf{(e)} we do \emph{not} claim the encoder/decoder $H(c\,|\,\mathrm{conf})$ gap holds outside our 8-task corpus. Each of these is a deliberate scope-limit, not a hedge: extending any of them is a defined v1.2 cell that can be added without retraining the existing 67 pools.
...
\paragraph{L2 — Decoder cross-pool predictability fails at our scale.} The fitted relation is plotted per pool in App.~\ref{app:scatter}. 25 decoder pools is too little variation across pools to extract a stable cross-pool LOPO regression signal for accuracy gain (encoder accuracy frontier $R^2{=}0.60$, decoder $R^2{=}{-}1.14$, Table~\ref{tab:frontier_strat}). Larger decoder pool counts may close this gap; we leave it open.
...
\paragraph{L16 — Our weight-space merges operate on LoRA factors, not on $\Delta W$.} \texttt{weight\_\allowbreak space\_\allowbreak merge.\allowbreak py} averages the \texttt{lora\_A} and \texttt{lora\_B} tensors independently, and $\mathrm{mean}(B)\,\mathrm{mean}(A) \neq \mathrm{mean}(BA)$, so the uniform/TIES/DARE numbers in App.~\ref{app:repro} are a factor-wise variant rather than the published methods. The prediction-space \texttt{uniform\_soup} proxy is unaffected (it is marked as a proxy).

\paragraph{L17 — Generation-family adapters were trained without label masking.} \texttt{real\_\allowbreak lora\_\allowbreak train.\allowbreak py} pads to \texttt{max\_length} and copies \texttt{input\_ids} into \texttt{labels}, so padding and prompt tokens contribute to the causal-LM loss. This affects the GSM8K pools only; the encoder classification pools and the multiple-choice trainer mask correctly.

\paragraph{Reproducibility.} Every released JSON carries the provenance block whose schema is given in App.~\ref{app:provenance}, the per-cell adjudication is specified as pseudocode in App.~\ref{app:pseudocode}, and the cell-by-cell coverage of the corpus is tabulated in App.~\ref{app:coverage}. Pool manifests, 1{,}028 per-cell JSONs (each carrying explicit \texttt{bh\_q\_value}, \texttt{adjudication\_\allowbreak pre/\allowbreak post\_\allowbreak fdr}, and a \texttt{bh\_batch\_hash} pinning the corpus-wide BH-FDR batch), ensemble results with per-example predictions/confidences, the per-adapter logits cache for the $6$ pools where it was retained (L15), all analysis code, and the locked Phase-0 pre-registration are released at an anonymous repository.\footnote{\url{https://anonymous.4open.science/r/AdapterFrontier/} (de-anonymized at acceptance).} \texttt{scripts/\allowbreak reproduce\_\allowbreak paper\_\allowbreak numbers.\allowbreak py} re-derives every headline number ($145$ assertions, ${<}30$\,s) from JSON alone, including both frontier tables. We characterise this as \emph{JSON-level reproducibility} (every claim re-computable from the release); adapter \emph{weights} are available on request for the minority of pools where they were retained (L15), via the anonymous repository's issues and will carry archival hashes at acceptance.

\section*{Ethical Considerations}

\paragraph{Compute footprint.} Training the 67 adapter pools required approximately $1{,}500$ GPU-hours total on a 50-node RTX 3060 cluster (orchestration in App.~\ref{app:cluster}) plus $\sim$$60$ A6000-hours for ensemble evaluation and serving benchmarks. PEFT adapters are intentionally compute-light ($\sim$$1$--$8$ GPU-hours each); the adapter weights we retained (L15) are available on request via the anonymous repository so that those pools need not be retrained. Estimated CO\textsubscript{2}e: approximately $0.18$ tonnes (RTX 3060 at $170$\,W $\times$ $1{,}500$\,h $+$ A6000 at $300$\,W $\times$ $60$\,h, average grid intensity).
...
\paragraph{Data characteristics, personal information and offensive content.} Every dataset we use is an established, publicly released English-language benchmark distributed by its original creators, all cited in \S\ref{sec:setup}: MNLI, QNLI and SST-2 (GLUE), BoolQ (SuperGLUE), AG~News, ANLI, HellaSwag and GSM8K. The domains are newswire, Wikipedia, movie reviews, crowd-written commonsense continuations and grade-school word problems. We collected no new data, recruited no annotators and ran no human subjects, so no consent or ethics-board procedure applies. We did not additionally screen these corpora for personally identifying information or offensive content and we do not claim they are free of it: they are widely used and their known characteristics are documented by their creators, and because our study measures only \emph{relative} calibration and accuracy between adapters trained and evaluated on identical examples, any such content affects both arms of every comparison equally. Readers reusing our released per-example predictions should consult the original dataset documentation rather than treat our release as a vetted corpus.
...
\appendix
% This paper has more than 26 appendices. LaTeX's \Alph stops at Z and writes
% an error macro into the label instead, which is how two appendix headings
% ended up lettered \GenericError in the .aux. Continue A..Z, AA, AB, ...
\makeatletter
\renewcommand\thesection{%
  \ifnum\value{section}>26 A\@Alph{\numexpr\value{section}-26\relax}%
  \else\@Alph\c@section\fi}
\makeatother
\sloppy
...
\paragraph{Setup.} Take one Pool-A RoBERTa HellaSwag adapter (seed 11, $lr{=}2{\times}10^{-5}$ HP-tuned). Evaluate on $5{,}000$-sample subsamples of HellaSwag validation drawn with \texttt{ds.\allowbreak shuffle(seed)} for $\text{seed}\!\in\!\{0, 11, 21, 42\}$; then on the full $10{,}042$-sample validation. Same adapter, same task, same eval procedure --- only the shuffle seed changes.
...
\paragraph{Models.} {\sloppy 10 base models from HuggingFace Hub: \texttt{bert-base-uncased}, \texttt{bert-large-uncased}, \texttt{roberta-base}, \texttt{microsoft/\allowbreak deberta-v3-base}, Qwen2.5 \texttt{\{0.5B, 1.5B, 3B\}}, \texttt{TinyLlama-1.1B-Chat-v1.0}, \texttt{SmolLM2-1.7B}, \texttt{pythia-1.4b}. Adapter weights (LoRA-A/B only, $\sim$20--300K trainable params per adapter) are released; we do \emph{not} redistribute base-model weights.\par}
\paragraph{Hyperparameters.} Each pool's HP grid is in \texttt{sweep\_\allowbreak configs/\allowbreak <pool>.\allowbreak json} (Pool-A: fixed HP, varying seed; Pool-B: rank$\in$\{4,8,16,32\} $\times$ alpha$\in$\{8,16,32\} $\times$ lr$\in$\{1e-4, 3e-4\}, fixed seed 42; Pool-C: \{LoRA, DoRA, rsLoRA, PiSSA\} $\times$ 5 seeds, fixed rank/alpha/lr). All runs use AdamW, cosine LR schedule with 6\% warmup, weight decay 0.01.
...
{\sloppy Three blockers we patched to run vLLM 0.10 on transformers 5.x: (i) transformers 5.x removed \texttt{all\_\allowbreak special\_\allowbreak tokens\_\allowbreak extended} --- patched in vLLM's tokenizer wrapper with an \texttt{hasattr} fallback; (ii) CUDA forking is rejected --- must export the vLLM worker-multiproc method as \texttt{spawn}; (iii) vLLM rejects PEFT adapters with \texttt{modules\_to\_save} (classification heads). We ship \texttt{strip\_\allowbreak modules\_\allowbreak to\_\allowbreak save.\allowbreak py} to make classification adapters serving-compatible. Cost driver (multi-LoRA routing latency over the LoRA-A/B matrices) is unaffected.\par}
...
\item v1.2.f ($2026$-$05$-$20$): full-validation re-evaluation of Pool-A RoBERTa-tuned and Pool-A DeBERTa-tuned HellaSwag pools on the entire $N{=}10042$ validation set; results released in \texttt{ensemble\_\allowbreak results/\allowbreak *\_\allowbreak fullval.\allowbreak json} (App.~\ref{app:splitvar}) $=$ $+8$ cells.
...
\item v1.3.f ($2026$-$06$-$17$): cross-corpus generalization stress test of the diversity-quality frontier regression (E2, \texttt{analysis/\allowbreak frontier\_\allowbreak cross\_\allowbreak corpus.\allowbreak json}). Encoder accuracy vs.\ \texttt{n\_rank} slice: Pool-A-trained ridge generalizes to Pool-B/C with $R^2_{\text{test}}{=}{+}0.62$; encoder-trained ridge generalizes to decoder pools with $R^2_{\text{test}}{=}{+}0.76$; random 50/50 within-encoder control $R^2_{\text{test}}{=}{+}0.48$. \texttt{best\_of\_n} slice including v1.1: v1.0-trained ridge generalizes to v1.1 HellaSwag/GSM8K with $R^2_{\text{test}}{=}{+}0.21$ (vs.\ random control $+0.25$). Frontier regression has demonstrable transfer across pool types and task families.
\item v1.3.g ($2026$-$06$-$17$): external reproducibility audit (E3). \texttt{scripts/\allowbreak reproduce\_\allowbreak paper\_\allowbreak numbers.\allowbreak py} re-derives the headline numerical claims (corpus counts, BH-FDR cutoffs, frontier $R^2$, $H(c|\mathrm{conf})$ gap, distillation recovery, vLLM serving ratios) from released JSON files only. As written on $2026$-$06$-$17$ the suite had $28$ assertions and two of them covered the 4-model contamination probes and the E1 ensemble probe; \textbf{those two were removed on $2026$-$07$-$28$} when v1.3.b and v1.3.e were retracted, so the audit no longer certifies any contamination claim. The suite now stands at $145$ assertions, all passing.
\item \textbf{v1.3.l ($2026$-$08$-$02$): the missing calibration control was run, and the headline claim changed.} Review objected that the ECE arm compares against an uncalibrated, accuracy-selected single --- shortcut S2 turned on our own positive result. \texttt{analysis/\allowbreak temperature\_\allowbreak control.\allowbreak py} fits one temperature on the pre-registered \texttt{val\_combine} split (reserved for combination hyperparameters, otherwise unused) for the six pools whose per-adapter logits cache survives, and measures on \texttt{test}. The ensemble goes from better-calibrated in $4/6$ pools (mean $\Delta$ECE ${+}0.008$) to $2/6$ (${-}0.032$); with both arms scaled it is $3/6$ (${+}0.002$). Median fitted $T{=}1.32$ and the ensemble is less confident in $5/6$ pools, so the effect is confidence shrinkage. Consequently the title, abstract, \S\ref{sec:intro-rules} framing, contribution (ii) and the \S\ref{sec:discussion} takeaway were rewritten: the paper no longer reports a dissociation, and the recommendation is now to calibrate one adapter rather than serve $N$. The $52$ is retained and reported with its scope. This is an amendment to the \emph{reading} of a pre-registered arm, not to the arm itself --- no cell, baseline or adjudication was recomputed.
\item \textbf{v1.3.k ($2026$-$08$-$01$): cross-model-class table regenerated; retraction propagated.} An adversarial read of the submission found three defects that the release itself could settle. (i) Table~\ref{tab:frontier_cross} reported ridge, random forest and gradient boosting per slice, but \texttt{frontier.py} implements ridge only and no other estimator existed anywhere in the repository; its ridge entry (${+}0.555$) also contradicted the headline (${+}0.597$) for the identical $192$-cell slice. \texttt{analysis/\allowbreak frontier\_\allowbreak model\_\allowbreak classes.\allowbreak py} now fits all three classes under \texttt{frontier.py}'s own slices, design matrix and leave-one-pool-out folds, delegating ridge to \texttt{frontier.\allowbreak lopo\_\allowbreak cv\_\allowbreak r2} so the headline column is the headline number by construction; the table is replaced by its output and $22$ new assertions pin every entry. The qualitative claim survives (ridge is the cross-class maximum on the headline slice and on no other), but the sentence asserting that ridge is \emph{never} the cross-class maximum was false on that slice and is corrected. (ii) v1.3.b declared the E1 ensemble probe void on $2026$-$07$-$28$, yet v1.3.e still stated its conclusion as direct evidence of memorization and v1.3.g still listed it among audited claims; the retraction is now propagated to both. (iii) Seven bibliography entries had been inserted into \S\ref{sec:related} instead of the bibliography, raising seven \texttt{Lonely \textbackslash item} errors and printing as prose; \S\ref{sec:setup}'s ``Tasks and models'' was an empty heading; and the D1--D5 rules the paper cites eight times were never enumerated. All three are fixed.
...
\item \textbf{v1.3.h ($2026$-$06$-$21$): independent audit response.} A read-only critique of the v1.3.g release flagged four classes of artifact-vs-claim drift: (i) corpus-wide BH-FDR claimed in the body was not present as an explicit per-cell field; (ii) \texttt{ensemble\_\allowbreak eval\_\allowbreak multichoice.\allowbreak py} performed best-single/greedy-subset selection on \texttt{val\_combine} instead of \texttt{val\_selection}; (iii) the \texttt{uniform\_soup} field in evaluator output was a prediction-space logit average, conflated with weight-space soup; (iv) \texttt{analysis/\allowbreak frontier.\allowbreak py} silently dropped rows with missing diversity features and included method-identity one-hot features, an untested confound. Fixes applied: (i) \texttt{scripts/\allowbreak apply\_\allowbreak corpus\_\allowbreak bh\_\allowbreak fdr.\allowbreak py} writes \texttt{bh\_q\_value}, \texttt{bh\_pass}, \texttt{adjudication\_\allowbreak pre\_\allowbreak fdr}, \texttt{adjudication\_\allowbreak post\_\allowbreak fdr}, and a corpus-wide \texttt{bh\_batch\_hash} into every per-cell JSON; corpus summary in \texttt{analysis/\allowbreak corpus\_\allowbreak bh\_\allowbreak fdr\_\allowbreak summary.\allowbreak json} (BH cutoff $p{=}0.0248$, $29.5\%$ SUPPORTED / $16.9\%$ REVERSED post-FDR, exactly matching abstract). (ii) Multichoice evaluator now uses \texttt{val\_selection} for all selection decisions and records \texttt{selection\_\allowbreak split=val\_\allowbreak selection} per method; rerun of affected HellaSwag cells requires the cluster adapter weights and is deferred to v1.4 alongside the clean-provenance refresh. (iii) Every method output now carries \texttt{method\_kind} (\texttt{prediction} / \texttt{selection}); \texttt{uniform\_soup} is explicitly marked \texttt{is\_proxy{=}True,\allowbreak\ proxy\_for{=}\allowbreak weight\_\allowbreak space\_\allowbreak uniform\_\allowbreak soup} — true weight-space soup is produced by \texttt{weight\_\allowbreak space\_\allowbreak merge.\allowbreak py} and reported separately. (iv) \texttt{frontier.py} now emits \texttt{drop\_\allowbreak diagnostics} (input/used/dropped row counts, drop reasons, per-pool/per-method drop breakdown) and an \texttt{ablation\_\allowbreak no\_\allowbreak method\_\allowbreak onehots} block; headline encoder accuracy slice has $0$ rows dropped and $|\Delta R^2|{=}0.0002$ between with/without method one-hots ($R^2_{\text{LOPO}}{=}{+}0.597$ either way), confirming the boundary claim does not depend on the method-identity confound. \texttt{scripts/\allowbreak reproduce\_\allowbreak paper\_\allowbreak numbers.\allowbreak py} grew from $28$ to $34$ assertions to verify these fields directly.
\item \textbf{v1.3.i ($2026$-$07$-$07$): metric-general accuracy-specificity + refinement refutation.} App.~\ref{app:mech} gains a paragraph establishing that the ECE non-result is metric-general, not an estimator artifact. \texttt{analysis/\allowbreak w2\_\allowbreak calibration\_\allowbreak asymmetry.\allowbreak py} recomputes the \texttt{n\_rank} LOPO frontier for five metrics from released per-adapter scalars: accuracy $R^2{=}{+}0.60$ but ECE $-0.06$ and Brier $-0.30$ (both unpredictable), while NLL $+0.42$ and MCE $+0.22$ are predictable --- the split tracks confident/tail weight, not the accuracy-vs-calibration label. \texttt{analysis/\allowbreak h3\_\allowbreak mechanism.\allowbreak py} tests and \emph{refutes} the natural refinement explanation: under the Murphy decomposition of the confidence-Brier, the resolution (refinement) gain is near-invariant ($\sigma$ $0.21{\times}$ that of accuracy) and uncorrelated with accuracy gain ($r{=}{-}0.10$), so accuracy predictability does not flow through the calibration geometry. Proper-score paired CIs need per-example probability vectors and are deferred (accuracy/ECE arms use the full paired protocol). \texttt{reproduce\_\allowbreak paper\_\allowbreak numbers.\allowbreak py} grew from $34$ to $64$ assertions; all $64$ pass.
\end{itemize}
...
(\texttt{analysis/\allowbreak correction\_\allowbreak sensitivity.\allowbreak py}).
...
The 50-node RTX 3060 12GB cluster is orchestrated via \texttt{sweep\_runner.py}, an in-house SSH-based fan-out engine. Each sweep config (JSON) declares a Cartesian parameter grid; the runner stages source files, assigns one (param-combination, node) pair per slot, and tracks per-job exit codes in \texttt{sweep\_\allowbreak runs/\allowbreak <name>/\allowbreak results.\allowbreak json}. Pool-A sweeps complete in $\sim$4--8 GPU-hours wall clock (parallelism factor $\sim$20); Pool-B/C in $\sim$16--24 hours. The v1.2 A6000 sweeps (Qwen-3B, Qwen-7B, Llama-3.1-8B-Instruct on GSM8K) use \texttt{local\_\allowbreak gpu\_\allowbreak runner.\allowbreak py}, a sequential per-seed launcher pinned to a single GPU index, suitable for models that do not fit on the 12GB cluster. Adapter weights are pulled back via \texttt{collect\_\allowbreak remote\_\allowbreak adapters.\allowbreak py} and assembled into pool manifests via \texttt{build\_\allowbreak adapter\_\allowbreak pool.\allowbreak py}; the manifest schema is documented in \texttt{scripts/\allowbreak validate\_\allowbreak pool.\allowbreak py}.

\section{Cost-savings analysis: derivation and stratified breakdown}\label{app:cost}

{\sloppy This appendix presents the cost-savings table (Table~\ref{tab:cost}) and stratifies by pool type and method. Regenerated by \texttt{analysis/cost\_savings.py}; groups are (pool, baseline kind, metric) over the $227$ non-self-comparison pools that carry \texttt{soft\_vote}, spanning $927$ cells: (A)~\emph{blind} --- always use \texttt{soft\_vote} (the default in much prior PEFT-ensembling work); (B)~\emph{routed} --- per-cell oracle pick over the four methods (\texttt{soft\_vote}, \texttt{majority\_vote}, \texttt{logit\_avg}, \texttt{greedy\_soup}) approximating D1--D5. Inference cost is single-adapter forward-passes per query: $N$ (pool size) for vote-based methods, $1$ for \texttt{greedy\_soup} (outputs a single merged checkpoint).\par}
...
\midrule
Enc acc (64)   & $-3.3$pp & $30\%$ & $20{\times}$ & $+0.4$pp & $8\%$ & $\mathbf{1{\times}}$ \\
Dec acc (50)   & $+0.6$pp & $0\%$  & $18{\times}$ & $+0.8$pp & $0\%$ & $15{\times}$ \\
Enc ECE (63)   & $+2.4$pp & $10\%$ & $20{\times}$ & $+3.6$pp & $2\%$ & $20{\times}$ \\
Dec ECE (50)   & $+3.5$pp & $0\%$  & $18{\times}$ & $+3.6$pp & $0\%$ & $18{\times}$ \\
\midrule
All (227)      & $+0.7$pp & $11\%$ & $19{\times}$ & $+2.1$pp & $3\%$ & $17{\times}$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Oracle method distribution.} The per-group argmax (Table~\ref{tab:cost}, ``D1--D5 routed'') picks \texttt{greedy\_soup} on $54$ of $114$ accuracy groups and $40$ of the $64$ encoder ones --- consistent with D1's preference for soup-merge on converged encoders --- against \texttt{majority\_vote} $22$, \texttt{logit\_avg} $22$ and \texttt{soft\_vote} $16$. Over all $227$ groups the counts are \texttt{soft\_vote} $104$, \texttt{greedy\_soup} $62$, \texttt{logit\_avg} $36$, \texttt{majority\_vote} $25$: probability averaging wins the calibration groups, soup-merge the encoder accuracy ones. This is an in-sample oracle (L11), so it bounds what rule-based routing could achieve rather than estimating it.

\paragraph{Deployable D1--D5 rule.} A deployable rule using only \emph{family} (encoder/decoder), \emph{convergence} (fraction of pool above chance), \emph{metric} (accuracy vs.\ ECE), and \emph{output space} (classification vs.\ generation) recovers most of the oracle gain in our cells; feature importances align with App.~\ref{app:features}. The remaining gap to per-cell oracle is the cost of not running \texttt{compute\_\allowbreak match.\allowbreak py} per-cell against the released $1{,}028$-cell baseline.
