...
\paragraph{Distillation works on Qwen-0.5B MNLI, \emph{fails} on Qwen-1.5B MNLI --- one-cell phenomenon.} Following the soft-target knowledge distillation framework \citep{hinton2015distilling} at $\alpha{=}0.9, T{=}4$: on \textbf{Pool-A Qwen-0.5B MNLI} the student attains acc 0.8479 / ECE 0.0397, recovering \textbf{87\%} of the soft-vote accuracy gain (vanilla single: 0.8365 / 0.0465; soft-vote: 0.8495 / 0.0187). On \textbf{Pool-A Qwen-1.5B MNLI} (same $\alpha, T$): student acc 0.8777 / ECE 0.0782 vs.\ vanilla single 0.8772 / 0.0768, soft-vote 0.8884 / 0.0454 --- \emph{the student recovers only $4.5\%$ of the accuracy gain and \emph{worsens} ECE}. The 87\% recovery on Qwen-0.5B does not generalize to Qwen-1.5B; reported as a $1$-of-$2$ tested decoder-cell positive, consistent with L11 (D1--D5 are heuristics, not generalization guarantees). Encoder distillation (Pool-A bert-base) also fails on both arms, consistent with encoder-accuracy-REVERSED (\S\ref{sec:findings}). Pool-A accuracy plateaus by $N{\approx}8{-}16$ across all model classes; ECE continues to decline modestly to $N{=}20$ in non-saturated pools (App.~\ref{app:scaling}, Fig.~\ref{fig:scaling}). On all 19 MNLI-trained pools (Pool-A/B/C) evaluated on ANLI dev\_r3 (OOD), most land near chance ($0.28$--$0.42$); soft-vote lowers OOD ECE on $14/19$ pools (mean $\Delta$ECE${=}{-}0.021$), but accuracy gain is positive on \textbf{only 5/19} (4/7 Pool-A, 0/6 Pool-B, 1/6 Pool-C): \emph{HP-diverse and method-mixed pools consistently hurt OOD accuracy} (App.~\ref{app:ood}).
...
\paragraph{L12 — GSM8K pre-training contamination is substantial; ensemble gains sit on memorized items.} A v1.3 digit-renumbering probe (100 GSM8K test items, every integer shifted by $+7$ in both question and gold answer) on Pool-A seed\_11 adapters reveals catastrophic accuracy drops: \textbf{Qwen-2.5-7B $0.66{\to}0.04$ ($\Delta{=}{-}62$pp)}; \textbf{Qwen-2.5-3B $0.61{\to}0.05$ ($\Delta{=}{-}56$pp)} (\texttt{analysis/gsm8k\_contamination\_probe\{,\_3b\}.json}). Both models' "math reasoning" is largely \emph{memorization of GSM8K-shaped items}, not surface-form-robust arithmetic. The within-Qwen monotone scaling ($+2.8/+5.4/+8.9/+8.5$pp) and the cross-family inverse pattern ($r{=}{-}0.98$ on $n{=}3$) reported in \S\ref{sec:findings}~(P$_c$) are therefore consistent with two mechanisms: ensemble disagreement on residual non-memorized items \emph{or} differential memorization across base models. Mistral-7B-v0.1's strongest +$13.4$pp is partial counter-evidence for pure-contamination but does not rule out memorization. \emph{Conservative reading}: the (P$_c$) decoder gains should be read as "ensemble effects on a contaminated benchmark"; the underlying arithmetic-reasoning claim awaits a decontaminated probe (digit-shift, paraphrase, or fresh benchmark such as MATH or HumanEval-Math).

\paragraph{Reproducibility.} Pool manifests, 1{,}028 cell JSONs, ensemble results (with per-example confidence for bootstrap), all analysis code, and the locked Phase-0 pre-registration are released at an anonymous repository.\footnote{\url{https://anonymous.4open.science/r/AdapterFrontier-XXXX/} (de-anonymized at acceptance).} Every result JSON carries a \texttt{provenance} block (git SHA, config hash, library versions) for byte-level reproducibility.
...
\item v1.3.b ($2026$-$05$-$22$): GSM8K digit-renumbering contamination probe (R5 audit request). Pool-A seed\_11 on Qwen-2.5-7B: orig acc $0.66 \to$ shift acc $0.04$ ($\Delta{=}{-}62$pp). Pool-A seed\_11 on Qwen-2.5-3B: $0.61 \to 0.05$ ($\Delta{=}{-}56$pp). Both indicate heavy pre-training memorization of GSM8K; (P$_c$) decoder gains should be read as ``ensemble effect on the contaminated benchmark'' (L12).
