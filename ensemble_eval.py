...
def main() -> int:
    ...
    # Per-adapter predictions on val_selection (schema v2, C1 prereg §4):
    # prospective-policy features must be computable from validation-side
    # artifacts only. Without these fields, downstream feature extraction
    # would be forced onto test-side individual_* fields — prohibited leakage.
    individual_predictions_val_selection = []
    individual_confidence_val_selection = []
    for i in range(len(adapters)):
        sel_probs = _softmax(logits_list[i][sel_idx])
        individual_predictions_val_selection.append(sel_probs.argmax(axis=-1).astype(int).tolist())
        individual_confidence_val_selection.append(sel_probs.max(axis=-1).astype(float).tolist())
    ...
    results: dict = {
        ...
        "schema_version": 2,  # v2: + val_selection/val_combine indices & val-side per-adapter preds
        "task": args.task,
        ...
        "individual_accuracy_val_selection": individual_acc_on_sel.tolist(),
        "individual_predictions_val_selection": individual_predictions_val_selection,
        "individual_confidence_val_selection": individual_confidence_val_selection,
        "labels_test": labels_test.astype(int).tolist(),
        "labels_val_selection": labels_sel.astype(int).tolist(),
        "test_indices": test_idx.astype(int).tolist(),
        "val_selection_indices": sel_idx.astype(int).tolist(),
        "val_combine_indices": combo_idx.astype(int).tolist(),
    ...
    for m in methods:
        ...
        # method_kind documents what *space* the aggregation acts on (audit
        # fix Critical 2). All methods in this evaluator are prediction-space:
        # they aggregate per-adapter logits/probabilities computed from
        # independent forward passes. True weight-space methods (uniform
        # weight average, TIES, DARE) live in weight_space_merge.py and
        # write merged adapters under merged_adapters/. The label
        # uniform_soup here is a prediction-space PROXY and MUST NOT be
        # cited as a weight-space soup result.
        if m in ("majority_vote", "soft_vote", "logit_avg", "uniform_soup"):
            method_meta["method_kind"] = "prediction"
            if m == "uniform_soup":
                method_meta["note"] = (
                    "PROXY: prediction-space uniform logit average. This is NOT "
                    "the weight-space LoRA soup; real weight-space results are "
                    "produced by weight_space_merge.py and stored under "
                    "merged_adapters/. Reviewers comparing against weight-space "
                    "soup must use those separate artifacts."
                )
                method_meta["proxy_for"] = "weight_space_uniform_soup"
                method_meta["is_proxy"] = True
        elif m == "greedy_soup":
            method_meta["method_kind"] = "prediction"
            ...
            method_meta["selection_split"] = "val_selection"
        elif m == "best_single":
            method_meta["method_kind"] = "selection"
            ...
            method_meta["selection_split"] = "test (sanity only)"
        else:
